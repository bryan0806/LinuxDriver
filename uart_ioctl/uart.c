...
int uart_ioctl (struct inode *Inode, struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    case IOCTL_SET_BAUD:
	 ...
	 // Set Baud
	 reg=inb(PORT_COM1+REG_LCR);// 先讀取LCR Port 為了讓原本有的設定保留下
	 outb( (reg|DLR_ON),(PORT_COM1+REG_LCR) );//除了DLR_ON的設定 也讓原本的reg設定保留下來
	 outw(divisor,PORT_COM1);// SET baud rate
	 outb( (reg & (~DLR_ON) ), PORT_COM1+REG_LCR);// DLR_ON位元清為0
         break;
    default:
